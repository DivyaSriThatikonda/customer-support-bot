...
@st.cache_resource
def load_agent(document_content):
    return SupportBotAgent(document_text=document_content)
    ...
    if st.button("🚀 Process Document"):
        if uploaded_file:
            with st.spinner("Processing document... This may take a moment."):
                ...
                if document_text:
                    st.session_state.agent = load_agent(document_text)
...
# Initial message if no agent is loaded yet
...
if prompt := st.chat_input("Ask a question about your document..."):
    ...
    with st.chat_message("user"): # Use Streamlit's native chat message for consistency
        st.markdown(prompt)

    # Check the state and respond with logic
    if st.session_state.agent:
        # If agent is ready, run the full workflow
        ...
        st.rerun() # Rerun to display the agent's full response
            
    elif uploaded_file:
        # If a file is uploaded but not processed
        st.warning("Please click the 'Process Document' button in the sidebar before asking a question.")
        st.session_state.messages.append({"role": "assistant", "content": "Please click the 'Process Document' button in the sidebar before asking a question."})
        st.rerun()
    else:
        # If no file has been uploaded at all
        st.warning("Please upload a document and click 'Process Document' first.")
        st.session_state.messages.append({"role": "assistant", "content": "Please upload a document and click 'Process Document' first."})
        st.rerun()
